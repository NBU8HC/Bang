...
try:
    from update_parameter.dcm_parameter_tool import ParameterUpdateTool
except ImportError:
    ParameterUpdateTool = None
...
class HomeScreen:
    ...
    def create_widgets(self):
        tk.Label(self.root, text="Pick your needed tool pls", font=("Arial", 14, "bold"), pady=20).pack()
    ...
    def open_update_parameter(self):
        if ParameterUpdateTool:
            win = tk.Toplevel(self.root)
            # win.withdraw()
            ParameterUpdateTool(win)
        else:
