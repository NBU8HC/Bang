...
try:
    from Split_parameter_tool.split_parameter import main as split_parameter_main
except ImportError:
    split_parameter_main = None
try:
    from update_parameter.dcm_parameter_tool import main as update_parameter_main
except ImportError:
    update_parameter_main = None
...
class HomeScreen:
    ...
    def open_split_parameter(self):
        if split_parameter_main:
            split_parameter_main()  # Gọi hàm main của tool Split Parameter
        else:
            messagebox.showerror("Error", "Không tìm thấy tool Split Parameter")

    def open_update_parameter(self):
        if update_parameter_main:
            update_parameter_main()  # Gọi hàm main của tool Update Parameter
        else:
